...
int main(void)
{
    ...
    */
    ...
    // accept() crea un nuovo socket per ogni singola connessione, tenendo quello originale per rispondere alle altre
    ...
    // 5 - Comunicazione vera e propria

    char buffer[4096];
    memset(buffer, 0, sizeof(buffer));

    // Ricezione dati con recv(), restituisce un numero intero:
        // - Se > 0 sono le lettere ( byte ) ricevuti
        // - Se = 0 il client ha chiuso la comunicazione
        // - Se < 0 errore


    int bytesRicevuti = recv(clientFd, buffer, sizeof(buffer), 0);

    char res[] = "200 OK - end comunication";
    if (bytesRicevuti > 0)
    {
        std::cout << "Messaggio ricevuto: \n" << buffer << std::endl;
        // Rimandiamo un messaggio
